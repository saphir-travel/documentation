...
.. warning::
    In the older versions, when the ``selected_value`` was not present, there were no ``words``
    associated. In the new version, the ``words`` key is always present and contains all the
    candidates.

    Version introducing ``words`` when ``selected_value`` is not present:
     - invoices: 121
     - expenses: 131
     - applicant: 101
